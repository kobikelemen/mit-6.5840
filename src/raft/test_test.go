...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		DPrintf("DISCONNECTED: %v %v %v", i1, i2, i3)
		...
		cfg.connect(i2)
		cfg.connect(i3)
		DPrintf("RECONNECTED: %v %v %v", i1, i2, i3)
	}
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf("DISCONNECTED S%v", (leader+1) % servers)
	...
	DPrintf("RECONNECTED S%v", (leader+1) % servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	DPrintf("RECONNECTED %v", (leader + 2) % servers)
	cfg.connect((leader + 3) % servers)
	DPrintf("RECONNECTED %v", (leader + 3) % servers)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("DISCONNECTING %v", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("DISCONNECTING %v", leader2)
	cfg.disconnect(leader2)

	// old leader connected again
	DPrintf("RECONNECTING %v", leader1)
	cfg.connect(leader1)

	cfg.one(104, 2, true)

	// all together now
	DPrintf("RECONNECTING %v", leader2)
	cfg.connect(leader2)
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			DPrintf("DISCONNECTING %v", victim)
			cfg.disconnect(victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			DPrintf("CRASHING %v", victim)
			cfg.crash1(victim)
			cfg.one(rand.Int(), servers-1, true)
		}
